...
for i in range(len(band)):
    ...
    sBVcur = SNsBV[i][np.absolute(SNM[i]-fitvals)<disp[i]]
    SNMcur = SNM[i][np.absolute(SNM[i]-fitvals)<disp[i]]
    sBVcur_err = SNsBV_err[i][np.absolute(SNM[i]-fitvals)<disp[i]]
    SNMcur_err = SNM_err[i][np.absolute(SNM[i]-fitvals)<disp[i]]
