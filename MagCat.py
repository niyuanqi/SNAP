...
#           reference star catalog. Unlike MagCalc which treats #
#           multiple objects as a combined multi-obj fit,       #
#           multiple objects are treated as a catalog of        #
#           sources for which we obtain fluxes independently.   #
#################################################################
...
def magnitude(image, catimage, wcs, cat, catname, (RAo,DECo), radius=500, over_intens=None, aperture=None, psf='1', name='object', band='V', fwhm=5.0, fsize=3, limsnr=3.0, satmag=14.0, refmag=19.0, fitsky=True, satpix=40000.0, verbosity=0, fitact="", corr_ast=None, diagnosis=False, solution=False):
    ...
    if verbosity > 3:
        ...
        plt.plot([Xo[0]-radius/2, Xo[0]+radius/2],
                 [Yo[0]+radius/2, Yo[0]+radius/2], 'r')
        plt.scatter(catX, catY, marker='o', edgecolor='b', facecolor='None')
    ...
    #calculate photometry for each source object
    #extract PSF to as great a degree as needed from source
    Io,SNo,skyNo = np.zeros(Nobj),np.zeros(Nobj),np.zeros(Nobj)
    PSFpopt, PSFperr = np.zeros((Nobj, 7)), np.zeros((Nobj, 7))
    for i in range(Nobj):
        PSFpopti, PSFperri, X2dof, skypopti, skyNi = [0]*7, [0]*7, 0, [0]*3, 0
        if verbosity > 0:
            print "Computing photometry of source "+name[i]

        if psf[i] == '1':
            PSFpopti, PSFperri, X2dof, skypopti, skyNi = pht.PSFscale(image, catPSF, catPSFerr, Xo[i], Yo[i], fitsky=fitsky[i], sat=satpix, verbosity=verbosity)
        elif psf[i] == '2':
            PSFpopti, PSFperri, X2dof, skypopti, skyNi = pht.PSFfit(image, catPSF, catPSFerr, Xo[i], Yo[i], fitsky=fitsky[i], sat=satpix, verbosity=verbosity)
        elif psf[i] == '3':
            PSFpopti, PSFperri, X2dof, skypopti, skyNi = pht.PSFextract(image, Xo[i], Yo[i], fwhm, fitsky=fitsky[i], sat=satpix, verbosity=verbosity)
        
        PSFpopt[i], PSFperr[i], skyNo[i] = PSFpopti, PSFperri, skyNi
        #check preferred intensity calculation method
        if aperture is None:
            #integrate PSF directly
            Io[i], SNo[i] = pht.PSF_photometry(image, Xo[i], Yo[i], PSFpopt[i], PSFperr[i], psf[i], skypopti, skyNo[i], verbosity=verbosity)
        else:
            #perform aperture photometry
            if aperture <= 0:
                #use FWHM of catPSF to define Kron aperture
                Io[i], SNo[i] = pht.Ap_photometry(image, Xo[i], Yo[i], skypopti, skyNo[i], PSF=catPSF, fitsky=fitsky[i], verbosity=verbosity)
            else:
                #use aperture given directly
                Io[i], SNo[i] = pht.Ap_photometry(image, Xo[i], Yo[i], skypopti, skyNo[i], radius=aperture, fitsky=fitsky[i], verbosity=verbosity)
    
    #Process each source
    I,RAo,DECo,mo,mo_err,mlimo = np.zeros(Nobj),np.zeros(Nobj),np.zeros(Nobj),np.zeros(Nobj),np.zeros(Nobj),np.zeros(Nobj)
    for i in range(Nobj):
        #check if source is valid
        if Io[i] != 0 and SNo[i] != 0 and skyNo[i] != 0:
        ...
        if I[i] != float('NaN') and I[i] > 0 and skyNo[i] != 0:
        ...
        if limsnr != 0 and skyNo[i] != 0:
            #sky noise properly estimated, calculate limiting magnitude
            ru = 10.0 #recursion seed
            rl = 0.1 #recursion seed
            mlim, SNlim, expu, expd = limitingM(ru, rl, limsnr, catPSF, catPSFerr, skyNo[i], catMags, catMagerrs, catSNs, catIs, verbosity)
            mlimo[i] = mlim
        elif limsnr != 0:
    ...
    if limsnr != 0:
        ...
        return RAo, DECo, I, SNo, mo, mo_err, mlimo
    else:
